...
static void emit(int c){
  ...
  if(n==1||D<mn[c]){mn[c]=D;argmn[c]=n; if(c==TRACK && !TRACKMAX) printf("rec min c=%d n=%lld pos=%lld D=%.12Lf\n",c,n,pos,D);}
  if(n==1||D>mx[c]){mx[c]=D;argmx[c]=n; if(c==TRACK && TRACKMAX) printf("rec max c=%d n=%lld pos=%lld D=%.12Lf\n",c,n,pos,D);}
