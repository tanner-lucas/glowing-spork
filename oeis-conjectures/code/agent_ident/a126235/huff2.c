...
int main(int argc,char**argv){
  ...
  int list[200000]; int cnt=0; for(int k=1;k<=24;k++){ for(int d=-3;d<=3;d++){ ll v=3LL*(1LL<<k)-1+d; if(v>=2 && v<=N) list[cnt++]=v; } } srand(1); for(int r=0;r<150;r++){ list[cnt++]=2+ (int)(((double)rand()/RAND_MAX)*(N-2)); }
  for(int q=0;q<cnt;q++){ int n=list[q];
    int res[2];
